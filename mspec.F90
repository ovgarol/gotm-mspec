...
!!
!! This version is modified to the 1D GTOM-FABM simulation
!!--------------------------------------------------------------
module gotm_mspec
  use fabm_types
  implicit none
  ...
  real(rk), parameter :: eps = 0.001_rk ! Small parameter to provide division by zero
...
! --- HZG model types
  type type_mspec_var
    ...
    real(rk) :: par,SeaTemp,DeepWtemp,pCO2,cil_h,cil_l,copepod_l,copepod_h !Forcings
  end type
 ...
 ! end type
  ...
  type, extends(type_base_model), public :: type_gotm_mspec
   ...
   ! type(type_state_variable_id),dimension(numberofphytos) :: id_Phy,id_Q_N,id_Q_P
    type(type_state_variable_id),allocatable :: id_Phy(:),id_Q_N(:),id_Q_P(:)
  ...
  !  type(type_diagnostic_variable_id),dimension(numberofzoos) :: id_I_max
    type(type_diagnostic_variable_id),allocatable :: id_fPAR(:),id_fCO2(:),id_grazing(:),id_uptake_N(:),id_uptake_P(:),id_growth(:),id_sizespec(:)
    type(type_diagnostic_variable_id),allocatable :: id_I_max(:)
  ...
  !  real(rk),dimension(numberofphytos) ::  Q_N_initial, Q_P_initial,Phy_initial
    real(rk),allocatable :: Phy_initial(:), Q_N_initial(:), Q_P_initial(:) !OG
    integer :: rc
  ...
  !  real(rk),dimension(numberofphytos) :: log_ESD
    real(rk),allocatable :: sel(:),Lz_star(:),Lz(:)
    real(rk),allocatable :: log_ESD(:)
  ...
  !  real(rk), dimension(numberofphytos) :: mumax, N_affin, P_affin, QC, QN_min, QN_max, QP_min, QP_max, vN_max, vP_max
    real(rk),allocatable :: mumax(:), N_affin(:), P_affin(:), QC(:), QN_min(:), QN_max(:), QP_min(:), QP_max(:), vN_max(:), vP_max(:)
...
!> **Model parameters, descriptions and corresponding symbols used in formulas:**
  ...
  subroutine initialize(self, configunit)
    ...
    real(rk),allocatable :: Phy_initial(:),Phyto0(:),log_ESD(:) ! Phytoplankton biomass
    ...
    real(rk),allocatable :: Q_N_initial(:) ! Phytoplankton intracellular nitrogen cell quota
    real(rk),allocatable :: Q_P_initial(:) ! Phytoplankton intracellular phosphorous cell quota
    ...
    real(rk),allocatable :: sel(:) ! Selectivity, for each zooplankton add value to inside list
    real(rk),allocatable :: Lz(:) ! Zooplankton body size, log_e ESD, for each zooplankton add value to inside list
    ...
    real(rk),allocatable :: Lz_star(:) ! Optimal prey size, log_e ESD, for each zooplankton add value to inside list
    ...
    !write(0,*) 'zoo_num:',zoo_num
    allocate(self%id_Phy(phyto_num),self%id_Q_N(phyto_num),self%id_Q_P(phyto_num),stat=error)
    if (error.ne.0) then
       print*,'error: could not allocate memory for arrays'
       stop
    endif
    allocate(self%id_fPAR(phyto_num),self%id_fCO2(phyto_num),self%id_grazing(phyto_num),self%id_uptake_N(phyto_num), &
             self%id_uptake_P(phyto_num),self%id_growth(phyto_num),self%id_sizespec(phyto_num),stat=error)
    if (error.ne.0) then
       print*,'error: could not allocate memory for arrays'
       stop
    endif
    allocate(self%id_I_max(zoo_num),stat=error)
    if (error.ne.0) then
       print*,'error: could not allocate memory for arrays'
       stop
    endif
    allocate(self%Q_N_initial(phyto_num),self%Q_P_initial(phyto_num),self%Phy_initial(phyto_num),self%log_ESD(phyto_num),stat=error)
    if (error.ne.0) then
       print*,'error: could not allocate memory for arrays'
       stop
    endif
    ...
    allocate(self%sel(zoo_num),self%Lz_star(zoo_num),self%LZ(zoo_num),stat=error)
    if (error.ne.0) then
       print*,'error: could not allocate memory for arrays'
       stop
    endif
    ...
    allocate(self%mumax(phyto_num),self%N_affin(phyto_num),self%P_affin(phyto_num),self%QC(phyto_num),self%QN_min(phyto_num), &
             self%QN_max(phyto_num),self%QP_min(phyto_num),self%QP_max(phyto_num),self%vN_max(phyto_num),self%vP_max(phyto_num),stat=error)
    if (error.ne.0) then
       print*,'error: could not allocate memory for arrays'
       stop
    endif
    ...
    write(0,*) ' get scaling parameters ....'
    ...
    !  Write(0,*)'pars'//trim(int2char(ib)),self%pars(ib)
    end do
    ...
    write(0,*) 'Register state variables'
    do ib=1,phyto_num
...
subroutine do(self, _ARGUMENTS_DO_)
  class (type_gotm_mspec),intent(in) :: self
 ...
 !! Forcings------------------------------------------
  ...
  if(self%graz_forc .eqv. .true.) then
    ...
    do cb=1,self%num_ciliat
       cop_pref(cb)=exp(-self%sel_cop_cil*(self%Lz_star(self%zoo_num-1)+self%Lz_star(self%zoo_num)-Lz_tmp(cb))**2)
    end do
 ...
 ! csrA = 0.5
 ! csrW = 0.2
 ! csr = (1._rk-csrA)+csrA*(1.0_rk/(1.0_rk+exp(csrW*(dble(doy)))))
 ! mixl = self%z*csr
 ...
 !  Retrieve current environmental conditions
  ...
  !write(0,*) 'PAR read from gotm:',par
  ...
  !write(0,*) 'PAR read from gotm:',par
  if(self%PAR_forc .eqv. .true.) call f_parsr(self,Phy,Q_N,f_co2,T_forcing,par,1.0_rk,F_par)
  !!--------------------------
  !write(0,*) 'f_par out:',F_par
  ...
  !! write(0,*) 'real growth:',rel_growthr
